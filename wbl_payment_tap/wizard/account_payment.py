...
class RefundAmount(models.TransientModel):
    ...
    transaction = fields.Many2one(string="Transaction ID",
                                  ...
                                  readonly=True)
    tap_transaction_id = fields.Char(string="Tap Transaction ID",
                                           related='relation_to.payment_transaction_id.tap_transaction_id',
                                           readonly=True)
    ...
    def action_send_refund(self):
        ...
        if refund_amount and refund_reason:
            payload = {
                "charge_id": self.tap_transaction_id,
                "amount": refund_amount,
                "currency": self.transaction.tap_transaction_currency,
            ...
            if response.status_code == 200:
                ...
                payment_data = response.json()
                raise ValidationError(f"Refund failed: {payment_data['errors'][0]['description']}")
        else:
